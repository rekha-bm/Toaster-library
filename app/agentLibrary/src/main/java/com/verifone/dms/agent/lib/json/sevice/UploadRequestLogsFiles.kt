...
/*To upload the File data to the server, we need to use the multipart form request. To do that,
we first create a subclass that inherits the Volley’s Request class.*/
open class VolleyFileUploadRequest(
  ...
  private val ending = "\r\n"
  private val boundary = "imageRequest${UUID.randomUUID()}"
...
class FileDataPart(val fileName: String?, val data: ByteArray, val type: String)
